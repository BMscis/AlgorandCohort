...
export const AwsProvider = ({ children }) => {
  ...
  /* async function saveNFT(nft) {
    const sv = await DataStore.save(new NFTS(nft));
  } */
  const awsStore = {
    authors,
    user,
  ...
  return <AwsContext.Provider value={awsStore}>{children}</AwsContext.Provider>;
};
